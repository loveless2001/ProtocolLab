...
1. **Mathematical Transition Soundness:**  
   The Bend functions `apply`, `fold`, and `summarize` satisfy all 12 declared laws in `LAWS.bend` under the type system, termination semantics, and reduction rules of the Bend 2.0.5 compiler.
...
1. **No Untrusted `verified=True` Flags:**  
   The Python shell and adapters must provide attributable receipts, SHA-256 payload digests, and explicit boundary grades (`local_generate_input`, `api_response_started`). The kernel never accepts an unverified boolean assertion of correctness.
